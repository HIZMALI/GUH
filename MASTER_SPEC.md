...
GridSentinel is an on-premise early-warning prototype for Grid Up Hackathon's panel/cell anomaly challenge. It combines existing MPR-53CS and ABB TVOC-2-COM monitoring with proposed retrofit environmental sensors and derived PD features. No field installation, real SCADA connection, certified hardware claim or breaker control. All demonstrations are synthetic.
...
V1 compatibility and dated evidence are retained. A single focused synthetic panel advances at configurable fast cadence while the background fleet remains paced within its limit. Durable scenario revision/run identity separates current and full history without deletion; structured early-warning transitions use a centralized safe action policy. New UI defaults to the current run and exposes full history on request; a pending new run never presents an old critical snapshot as new data.

Three read-only TCP banks provide all 500 panels: 1502/1..247,1503/1..247,1504/1..6. V1 device maps and output register values are retained. H1/TH-A occupies the right auxiliary air space in every drawing. The repository includes an EDA-neutral reference carrier and host-testable MCU firmware; actual EDA/embedded build status must be reported honestly. Tiered deployment/TCO, design rationale, an environmental/EMC plan, official requirements traceability and a technical demo walkthrough document the implementation. Recorded regression, load and focused combined/arc measurements define the verified scope. The normal demo state contains 500 panels with no offline panel or current open alarm.
