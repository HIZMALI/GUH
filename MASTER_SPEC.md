...
## V2 hackathon readiness delta
Preserve v1 behavior and evidence. A single focused synthetic panel advances at configurable fast cadence while the background fleet remains paced within its limit. Add explicit durable scenario revision/run identity, separate current/full history without deletion, structured early-warning transitions and centralized safe action policy. New UI defaults to the current run and exposes full history on request; a pending new run never presents an old critical snapshot as new data.

Three read-only TCP banks provide all 500 panels: 1502/1..247,1503/1..247,1504/1..6. Keep v1 device maps and output register values. H1/TH-A occupies the right auxiliary air space in every drawing. Add an EDA-neutral reference carrier and host-testable MCU firmware; actual EDA/embedded build status must be reported honestly. Add tiered deployment/TCO, innovation, environmental/EMC plan, official requirements traceability and a five-minute jury script. Re-run regression and load tests, measure focused combined/arc timing, preserve original v1 results, package a secret-free Git baseline delta, and leave 500 panels presentation-ready.
