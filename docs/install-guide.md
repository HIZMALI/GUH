...
Kaynak veri çıkarımını yeniden yapmak için orijinal PDF/Excel dosyalarının bulunduğu ayrı bir kopyada Python ortamına pypdf ve openpyxl kurup `python scripts/analyze_sources.py` ve `python scripts/import_dataset.py` çalıştırılabilir. Orijinaller güncel repository dağıtımına dahil değildir; [kaynak ve hash kontrolü](source-analysis.md#kaynak-dosyalarının-dağıtımı-ve-hash-kontrolü) ayrı belgelenmiştir. Normal demo, kontrol edilmiş JSON veriyle çalışır; workbook paketlerine runtime bağımlılığı yoktur.
