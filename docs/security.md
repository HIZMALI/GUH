...
DB kesintisinde başarılı ingest cevabı verilmez; publisher MQTT PUBACK'i DB transaction commit'i değildir. Subscriber kalıcı broker oturumu, bounded8192 frame kuyruğu ve transaction sonrası manual ACK uygular; geçici DB arızası tekrar denenir. V2 referans C++ edge kuyruğu host journal ile test edilir; fiziksel NVS/FRAM portu devreye alınmadı. [Firmware sözleşmesi](hardware/firmware.md). Uzun kesintide disk/broker retention, dolan kuyruk ve güç kaybı ayrıca doğrulanmalıdır.

V2 koşu kimliği tahminî zaman penceresinden türetilmez. Açık run/revision ve stable run-step mesaj kimliği kullanılır; eski V1 metadata alanları nullable kalır. Senaryo reset geçmişi silmez, audit korunur. Tek API süreci desteklenir: süreç içi kilit çoklu worker için yeterli değildir; yatay yayılım öncesi DB revision/focus/migration koordinasyonu gerekir. Yeni koşu pending iken UI değerleri beklemede, SCADA geçerlilik kapısı0'dır.
