...
## Read APIs
- Fleet `summary.open_alarms` counts only `active`/`acknowledged` alarms matching both their panel and its current `demo_run_id`; historical and legacy-unassigned alarms remain available in Alarm Center.
...
Policy version 2 keeps the ATTENTION dashboard/event/persistent alarm/SCADA/operator actions without SMS/WhatsApp records. New/escalated WARNING and CRITICAL alarms, and urgent ARC events, create both local mock channels; repeated frames do not resend. COMMUNICATION_LOSS uses availability/stale/connectivity actions. ARC adds a durable audit record and keeps ABB protection independent. Provider adapters cannot contact an external SMS/WhatsApp service. There is no breaker, protection trip/reset, TVOC reset or Modbus-write action.
