...
Paketleyici çalışma ağacı temiz değilse, hedef HEAD değilse veya yasaklı dosya varsa durur. Binary delta yalnız baseline ile final commit arasındadır; izole Git index'e uygulandıktan sonra tree hash'i gerçek final tree ile eşit olmalıdır. Orijinal PDF/XLSX, özel `.env`, bağımlılıklar, cache ve runtime çıktıları commit/delta dışında kalır; değişmeyen orijinal kaynaklar baseline'da korunur.
